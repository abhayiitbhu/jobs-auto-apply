    ...
    *,
    ...
    system = SystemMessage(
        content=(
            ...
            "ALWAYS mention the candidate's college/university (from the verified profile's "
            "education) by name in the letter — it is a prestigious institution and must never "
            "be omitted.\n"
            "Keep it SHORT and CRISP: flowing prose in exactly 3 tight paragraphs, no filler. "
            "Do NOT use emojis, bullet points, dashes as list markers, numbered lists, headings, "
            "or resume-style section labels (e.g. company names as headers or 'Tech used:' lines). "
            "It must read like a letter, not a resume.\n"
            "Use Markdown bold (wrap with **double asterisks**) for, and ONLY for: every company "
            "name (the target company and the candidate's past employers), the candidate's "
            "college/university name, and each key technical skill. Bold nothing else.\n"
            f"{ctc_rule}\n"
            f"Write in first person, {max_words} words or fewer (aim for far fewer). Open with a "
            "greeting addressed to the company/hiring manager and close with the candidate's name. "
            "Return only the cover letter text — no commentary, no quotes, and no Markdown other "
            "than the **bold** described above."
        )
    )
    human = HumanMessage(
        content=(
