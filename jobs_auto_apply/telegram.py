...
class TelegramClient:
    ...
    async def start(self) -> None:
        if not self.token:
            raise TelegramError(
                "telegram.bot_token is not set — create a bot with @BotFather and add the token"
            )
    ...
    async def open_chat(self) -> None:
        if not self.chat_id:
            raise TelegramError(
                "No Telegram chat_id yet. Run: python main.py telegram-login (then /start your bot)"
            )
    ...
    async def capture_chat_id(self, *, timeout: int = 120) -> str | None:
        ...
        while asyncio.get_event_loop().time() < deadline:
            text, chat = await self._next_message(long_poll=20)
            if chat:
                self.chat_id = chat
    ...
    async def wait_for_reply(self, *, timeout: int | None = None) -> str | None:
        ...
        while asyncio.get_event_loop().time() < deadline:
            ...
            text, chat = await self._next_message(long_poll=long_poll)
            if text is None:
                continue
            if self.chat_id and chat and chat != self.chat_id:
